...
### 31. In Pydantic's default mode, if a field is defined as `id: int` in a `BaseModel`, and input data provides `"123"` as a string, what happens during model instantiation?
...
### 32. When using `Field()` to set constraints like `ge=18` and `le=100` for an `age` field, what exact error type would occur if `age=101` is provided?
...
### 33. In the example of custom validators with `@field_validator("age")`, if `value=17` is passed, what is the precise exception raised inside the validator?
...
### 34. For serialization, calling `model_dump()` on a `User` model with `id=1` and `name="Ali"` returns a dict. How does `model_dump_json()` differ in output format?
...
### 35. In strict mode using `StrictInt` for field `x`, why does `Model(x="123")` fail while `Model(x=123)` succeeds?
...
### 36. When using aliases like `full_name: str = Field(..., alias="fullName")`, if data=`{"fullName": "Ali Khan"}`, what attribute access returns `"Ali Khan"`?
...
### 37. In Settings Management with `BaseSettings`, if `debug: bool = False` is defined, and no env var is set, what value does `s.debug` hold after instantiation?
...
### 38. Using `pydantic.dataclasses` on a standard `@dataclass` with `id: int`, if `id="123"` is passed, what occurs?
...
### 39. Calling `User.model_json_schema()` on a model with `id: int` and `name: str` generates what kind of output?
...
### 40. With `TypeAdapter(list[int])`, validating `["1", 2, 3]` returns `[1, 2, 3]`. What happens if `["1", "two", 3]` is passed?
...
### 41. In validation errors, for input `id="abc"` and `name=123`, the `errors()` list contains `loc` as `["id"]` and `["name"]`. What `msg` for `"name"`?
...
### 42. Why is Pydantic's core written in Rust mentioned in performance?
...
### 43. In models, if `signup_ts: datetime | None = None`, and data provides `"2024-01-01 10:30"`, it converts to datetime. What if omitted?
...
### 44. For `Field(min_length=3, max_length=20)` on `username`, if `"Ab"` is provided, what error details?
...
### 45. Custom validator with `@field_validator("age")` raises `ValueError` for `<18`. If `age=18`, what does it return?
...
### 46. Serialization with `model_dump_json()` on a model with datetime field outputs ISO format. What if `exclude_none=True`?
...
### 47. Strict vs Lax: In lax, `"123"` to int works. In strict with `StrictInt`, it fails. How to enable strict globally?
...
### 48. Aliases: If `alias="fullName"` for `full_name`, data uses `"fullName"`. What if data uses `"full_name"`?
...
### 49. In `BaseSettings`, `database_url: str` reads from env. If missing, what happens?
...
### 50. Pydantic dataclass with `id: int` converts `"123"` to `123`. How does it differ from `BaseModel`?
...
### 51. `model_json_schema()` output includes `"properties"` with types. For `id: int`, it's?
...
### 52. `TypeAdapter` for `list[int]` coerces strings to ints. If empty list `[]`, validates to?
...
### 53. Why use Pydantic over plain type hints?
...
### 54. In recap, `BaseModel` is for schemas. `Field` for rules. What subtle link?
...
### 55. If `User(id="abc")`, error `loc=["id"]`, `msg="Input should be a valid integer"`. If `id=1.5`?
...
### 56. Custom validator can modify value. If `return value + 1`, for `age=18` returns?
...
### 57. Serialization excludes private fields?
...
### 58. Strict mode for one field, lax for others possible?
...
### 59. Settings Management requires `pydantic-settings` install. For what?
...
### 60. `TypeAdapter` vs `BaseModel` for quick validations?
